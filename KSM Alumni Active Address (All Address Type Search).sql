...
Alt_Home As  (Select DISTINCT
      ...
      ,  a.state_code
      ,  A.COUNTRY_CODE
      , tms_country.short_desc as alt_home_country
      FROM address a
      INNER JOIN tms_addr_status ON tms_addr_status.addr_status_code = a.addr_status_code
      LEFT JOIN tms_address_type ON tms_address_type.addr_type_code = a.addr_type_code
      LEFT JOIN tms_country ON tms_country.country_code = a.country_code
...
Alt_Bus As  (Select DISTINCT
      ...
      ,  Listagg (a.city, ';  ') Within Group (Order By a.city) As city
      ,  a.state_code
      , tms_country.short_desc as alt_business_country
      FROM address a
      INNER JOIN tms_addr_status ON tms_addr_status.addr_status_code = a.addr_status_code
      LEFT JOIN tms_address_type ON tms_address_type.addr_type_code = a.addr_type_code
      LEFT JOIN tms_country ON tms_country.country_code = a.country_code
...
SELECT DISTINCT
  ...
  ,p.preferred_city
  ,p.preferred_country
  , home.home_city as home_non_pref_city
  , home.home_country as home_non_pref_country
  , Business.city as business_non_pref_city
  , Business.business_country as business_non_pref_country
  , Alt_Home.city as alt_home_city
  , Alt_Home.alt_home_country
  , Alt_Bus.city as alt_business_city
  , Alt_Bus.alt_business_country
   , Seasonal.city as seasonal_city
  , Seasonal.seasonal_country
  ,KD.CLASS_SECTION
