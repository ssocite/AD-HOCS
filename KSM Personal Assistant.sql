...
pa as      (select clean.id_number,
       max (clean.contact_id_number) keep (dense_rank first order by clean.date_modified DESC) as contact_id_number,

       max (clean.xcomment) keep (dense_rank first order by clean.date_modified DESC) as xcomment,

       max (clean.extract_email) keep (dense_rank first order by clean.date_modified DESC) as extract_email,

       max (clean.name) keep (dense_rank first order by clean.date_modified DESC) as names,

       max (clean.short_desc) keep (dense_rank first order by clean.date_modified DESC) as short_desc
from clean
